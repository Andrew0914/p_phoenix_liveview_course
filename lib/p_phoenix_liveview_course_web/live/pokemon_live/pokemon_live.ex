  ...
  defmodule PPhoenixLiveviewCourseWeb.PokemonLive do
    ...
    @impl true
    def handle_event("battle", _params, socket) do
      socket = battle(socket)

      # Dispara evento JS para animaciones
      {:noreply,
      socket
    ...
    @impl true
    def handle_event("battle_finished", _params, socket) do
      {:noreply, assign(socket, :show_result, true)}
    end

    @impl true
    def handle_info({:battle_finished}, socket) do
      {:noreply, assign(socket, :show_result, true)}
    end
    ...
    defp init_pokemons(socket) do
      ...
      socket
      |> assign(pokemons: available_pokemons, p1: nil, p2: nil, battle_result: nil, role: nil, show_result: false)
    end
